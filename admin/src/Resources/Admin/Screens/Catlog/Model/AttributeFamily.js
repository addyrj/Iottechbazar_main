...
import { attributeFamilyLanState, getAppLanguages, setLoder } from '../../../../../Database/Action/AdminAction';
...
const AttributeFamily = () => {
    ...
    const { attrFamilyLanState, attributeFamilySlug } = useSelector((state) => state.ConstantReducer);
    ...
    const updateLanguage = () => {
        if (isEmpty(attributeFamilySlug)) {
            toast.error("Failed! Attribute not found")
        } else if (isEmpty(multiCatName)) {
            toast.error("Failed! Text Language is not found")
        } else {
            let formData = new FormData();
            formData.append("attrFamilySlug", attributeFamilySlug);
            formData.append("mulLanguageName", JSON.stringify(multiCatName));
            dispatch(setLoder(true));
            axios.post(process.env.REACT_APP_BASE_URL + "addAttributeFamilyLanguage", formData, postHeaderWithToken)
                .then((res) => {
                    if (res.data.status === 200) {
                        dispatch(setLoder(false));
                        toast.success(res?.data?.message);
                        dispatch(attributeFamilyLanState({ attrFamilyState: false, attributeFamilySlug: "" }))
                    }
                })
                .catch((error) => {
    ...
    return (
        <Modal
            ...
            open={attrFamilyLanState}
            onClose={() => dispatch(attributeFamilyLanState({ attrFamilyState: false, attributeFamilySlug: "" }))}
                ...
                >
                    <div className="card mb-0">
                        <div
                            ...
                            <h3 className="card-title text-white col-9">Add Attribute Family Name</h3>
                            <button style={{
                                width: "200px",
                                height: "2.5rem",
                                color: "white",
                        ...
                        <form>
                            ...
                            >
                                ...
                                <button
                                    ...
                                    style={buttonStyle}
                                    onClick={() => dispatch(attributeFamilyLanState({ attrFamilyState: false, attributeFamilySlug: "" }))}
                                >
                                    Cancel
                                </button>
                            </div>
...
export default AttributeFamily
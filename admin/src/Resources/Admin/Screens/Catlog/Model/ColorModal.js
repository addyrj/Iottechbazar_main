...
import { changeColorLanState, getAppLanguages, setLoder } from '../../../../../Database/Action/AdminAction';
...
const ColorModal = () => {
    ...
    const { colorLanState, colorSlug } = useSelector((state) => state.ConstantReducer);
    ...
    const updateLanguage = () => {
        if (isEmpty(colorSlug)) {
            toast.error("Failed! Color not found")
        } else if (isEmpty(multiCatName)) {
            toast.error("Failed! Text Language is not found")
        } else {
            let formData = new FormData();
            formData.append("slug", colorSlug);
            formData.append("mulLanguageName", JSON.stringify(multiCatName));
            dispatch(setLoder(true));
            axios.post(process.env.REACT_APP_BASE_URL + "addColorLanguage", formData, postHeaderWithToken)
                .then((res) => {
                    if (res.data.status === 200) {
                        dispatch(setLoder(false));
                        toast.success(res?.data?.message);
                        dispatch(changeColorLanState({ colState: false, colSlug: "" }))
                    }
                })
                .catch((error) => {
    ...
    return (
        <Modal
            ...
            open={colorLanState}
            onClose={() => dispatch(changeColorLanState({ colState: false, colSlug: "" }))}
                ...
                >
                    <div className="card mb-0">
                        <div
                            ...
                            <h3 className="card-title text-white col-9">Add Color Name</h3>
                            <button style={{
                                width: "200px",
                                height: "2.5rem",
                                color: "white",
                        ...
                        <form>
                            ...
                            >
                                ...
                                <button
                                    ...
                                    style={buttonStyle}
                                    onClick={() => dispatch(changeColorLanState({ colState: false, colSlug: "" }))}
                                >
                                    Cancel
                                </button>
                            </div>

...
import { attributeFamilyLanState, getAllFamilyAttribute, setLoder } from '../../../../Database/Action/AdminAction';
...
const AdminAttributeFamily = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const [attributeData, setAttributeData] = useState([])
    const allFamilyAttribute = useSelector((state) => state.AdminReducer.allFamilyAttribute);
    const loader = useSelector((state) => state.AdminReducer.loader);

    const deleteAttributeFamily = (slug) => {
        let formData = new FormData();
        formData.append("slug", slug)
        dispatch(setLoder(true));
        axios.post(process.env.REACT_APP_BASE_URL + "deleteAttributeFamily", formData, postHeaderWithToken)
            .then((response) => {
    ...
    useEffect(() => {
        setAttributeData(allFamilyAttribute)
    }, [allFamilyAttribute])
                            ...
                            <div className="card">
                                <div className="card-body mb-4">
                                    <div className='page-event'>
                                        <NavLink to={"/admin_add_family_attributes"} className='buttonStyle'>Create Family</NavLink>
                                    </div>
                                    <table
                                        id="datatable"
                                        ...
                                        style={{ width: "100%" }}
                                    >
                                        {allFamilyAttribute?.length === 0 ? <div className='noDataStyle'>
                                            ...
                                            <>
                                                ...
                                                <tbody>
                                                    {allFamilyAttribute.map((currElem, index) => {
                                                        return (
                                                            <tr key={index}>
                                                                <td className="table-text-style">{currElem.id}</td>
                                                                <td className="table-text-style">{currElem.attributeName}</td>
                                                                ...
                                                                <td className="table-text-style text-center">
                                                                    <li className="nav-item dropdown" style={{ listStyleType: "none" }}>
                                                                        ...
                                                                        <div className="dropdown-menu dropdown-menu-lg dropdown-menu-right p-2">
                                                                            <NavLink to={"/admin_add_family_attributes"} className="dropdown-item mt-2 mb-1"
                                                                                state={{
                                                                                    slug: currElem.slug,
                                                                                    name: currElem.name,
                                                                            ...
                                                                            <a className="dropdown-item mt-2 mb-1" style={{ cursor: "pointer" }}>
                                                                                <div className="media"
                                                                                    onClick={() => deleteAttributeFamily(currElem.slug)}
                                                                                >
                                                                            ...
                                                                            <a className="dropdown-item mt-2 mb-1" style={{ cursor: "pointer" }}>
                                                                                <div className="media" onClick={() => dispatch(attributeFamilyLanState({ attrFamilyState: true, attributeFamilySlug: currElem.slug }))}
                                                                                >
...
export default AdminAttributeFamily